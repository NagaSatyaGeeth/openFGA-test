...
function makeClient({ apiUrl, apiKey }) {
  ...
  // The tuples that represent a single employee's identity in the
  // authority graph: their role (both directions - see model/model.fga's
  // comment on why) and their manager edge, if any. Used both for the
  // full-roster seed below and for incremental sync when the Users module
  // creates/edits one person at a time.
  function employeeTuples(e) {
    const tuples = [
      { user: `role:${e.role}`, relation: "role", object: `employee:${e.id}` },
      { user: `employee:${e.id}`, relation: "assignee", object: `role:${e.role}` },
    ];
    if (e.manager) {
  ...
  function buildTuples(org, employees) {
    const tuples = [];
    for (const e of employees || org.employees) {
      tuples.push(...employeeTuples(e));
    }
  ...
  // For the Users module: replace one employee's role/manager tuples in one
  // call (delete the old, write the new) so editing a person's role or
  // manager keeps the authority graph in sync immediately. Deleting a tuple
  // that no longer exists (e.g. first-time creation, no prior manager) is
  // OpenFGA's normal "not found" 400 - safe to ignore, same as a duplicate
  // write.
  async function replaceEmployeeTuples(storeId, oldEmployee, newEmployee) {
    const deletes = oldEmployee ? employeeTuples(oldEmployee) : [];
    const writes = employeeTuples(newEmployee);
    try {
      ...
      if (err.status === 400 && err.body && err.body.code === "write_failed_due_to_invalid_input") {
        // one or more individual tuples in this batch already existed / were
        // already absent - fall back to per-tuple so the *other* tuples in
        // the batch still land.
        for (const t of deletes) {
          try {
            await api("POST", `/stores/${storeId}/write`, { deletes: { tuple_keys: [t] } });
          } catch {}
        }
  ...
  return {
    ...
    buildTuples,
    employeeTuples,
    replaceEmployeeTuples,
    writeTuplesIdempotent,
    check,
    expand,
  };
}
